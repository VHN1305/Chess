...
def main():
    ...
    p.display.set_caption("Chess _ Nhom 10 _ Nhap Mon AI _ IT1 _ 05 _ K65 _ HUST")
        ...
        for e in p.event.get():
            if e.type == p.QUIT:
                ...
                if not gameOver and humanTurn:
                    ...
                    if sqSelected == (row, col) or col > 8: #pair<int, int> =
                        sqSelected = ()
                        playerClicks = []
                    else:
                        sqSelected = (row, col)
...
def hightlightSquares(screen, gs, validMoves, sqSelected):
    ...
    if len(gs.moveLog) > 1:
        secondLastMove = gs.moveLog[-2]
        s = p.Surface((SQ_SIZE, SQ_SIZE))
        s.set_alpha(255)
        s.fill(p.Color("Light Blue"))
        screen.blit(s, (secondLastMove.endCol * SQ_SIZE, secondLastMove.endRow * SQ_SIZE))
    if sqSelected != ():
        r, c = sqSelected
